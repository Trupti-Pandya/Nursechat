...
def find_sub_links(soup, base_url):
    ...
    # This is targeting the specific navigation pattern in the NHS website
    ...
    for nav in nav_elements:
        ...
        for link in links:
            ...
            if full_url.startswith(BASE_URL) and full_url not in sub_links:
                # Check if the link text contains meaningful content and isn't just an image alt text
                link_text = link.text.strip()
                if link_text and not link_text.startswith('Image:') and not link_text.lower() == 'image':
                    sub_links.append({
                        'url': full_url,
                        'text': link_text
                    })
...
def scrape_condition_page(url, depth=0, max_depth=1, visited=None):
    ...
    for attempt in range(3):
        try:
            ...
            # Recursively scrape sub-pages if we're not too deep
            sub_pages_content = ""
            if depth < max_depth:
                for link in sub_links:
                    sub_url = link['url']
